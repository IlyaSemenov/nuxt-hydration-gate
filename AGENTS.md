# nuxt-hydration-gate Agent Guide
...
- Keep Nuxt runtime code in `src/runtime/`, the Nuxt module entry in `src/module.ts`, and the Playwright entry in `src/playwright.ts`.
- Keep `src/playwright.ts` free of Nuxt runtime imports.
- Keep integration tests and the Nuxt fixture app in `tests/`.
- Treat the attribute name, `package.json` exports, and supported runtimes as public contracts.

## Playwright

- Keep the fixture's locator handler empty and let Playwright wait for the locator to be hidden, so the wait counts toward the action's timeout.
  Waiting inside the handler is unbounded by the action timeout and leaves a pending call that fails the test when it ends.
...
- Run the `test` script when behavior changes; it builds the package and the SSR and SPA fixture apps, then runs Playwright.
